...
pvals <-rdadapt_env$p.values
SNPs <- colnames(trueRDA$Ybar)
CHR <- unlist(lapply(strsplit(SNPs, split = "\\."), function(x) x[1]))
POS <- unlist(lapply(strsplit(SNPs, split = "\\."), function(x) x[2]))
all <- cbind(CHR, POS, pvals,pv)
#write.csv(rdadapt_env$p.values, paste0(outdir, "/permut", permut,"/rdadapt_pvalues.csv"), row.names = FALSE)

write.table(all, "/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/LandscapeGenomicsPipeline/RDA_utils/RDA_ResearchPlan_FilterMethod1/permutations_geo_control/merged_permutation_pvalues_true.csv", sep="\t", col.names = FALSE, row.names = FALSE, quote= FALSE)#MeanPermPval<-rowMeans(pv)
...
##include real pvalues
p <- read.csv("/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/LandscapeGenomicsPipeline/RDA_utils/RDA_ResearchPlan_FilterMethod1/AssociationAnalysis/pvalues.csv", h=TRUE)
all <- cbind(p,pv)
write.csv(all, "/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/LandscapeGenomicsPipeline/RDA_utils/RDA_ResearchPlan_FilterMethod1/permutations_geo_control/merged_permutation_pvalues.csv", col.names=FALSE)
