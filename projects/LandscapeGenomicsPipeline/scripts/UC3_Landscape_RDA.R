...
library(raster)
library(ggrepel)
###TROUBLE, not working due to C compilation
...
#meta <- as.data.frame(cbind(meta$sampleId, meta$continent, meta$country, meta$province, meta$lat, meta$long))
#colnames(meta) <- c("sampleId", "continent", "country", "province", "lat", "long")
#print(meta)
#af_file="/media/inter/ssteindl/FC/usecaserepo/uc3-drosophola-genetics/projects/LandscapeGenomicsPipeline/Europe50k_newSNPs/Subsampled3.af"
#af_file="/media/inter/ssteindl/FC/usecaserepo/uc3-drosophola-genetics/projects/LandscapeGenomicsPipeline/FullData/results/fullgenome2/Subsampled_fullgenome2.final_DP15.af"
...
print("Printing: matched.meta")

#print(matchedMeta$lat)
###SNPs with a minor allele frequency inferior to 5% were filtered out to avoid giving too much importance to rare alleles when
#looking for loci associated with environmental variation.Doing so means assuming that local adaptation is driven
# by consequent changes in adaptive allele frequency along environmental gradients.
...
library(patchwork)
a <- fviz_cos2(Env.PCA, choice = "var", axes = 1) + scale_x_discrete(labels = new_labels)
b <- fviz_cos2(Env.PCA, choice = "var", axes = 2) + scale_x_discrete(labels = new_labels)
c <- fviz_cos2(Env.PCA, choice = "var", axes = 3) + scale_x_discrete(labels = new_labels)
d <- fviz_cos2(Env.PCA, choice = "var", axes = 4) + scale_x_discrete(labels = new_labels)
panel <- a + b + c + d 
ggsave(paste0(outdir,"/VariablePartitions.png"), plot = panel, width = 8, height = 6, dpi = 300)
...
VariablesN <- Variables[,2:ncol(Variables)]

##included
column_names <- colnames(Variables)
...
formula_string <- paste("AllFreq ~ Latitude + Longitude +", paste(comp_columns, collapse = " + "))
# Convert the string to a formula
formula <- as.formula(formula_string)
AllFreq <- AllFreq[rownames(AllFreq) %in% Variables$sampleId,]
...
formula_string <-paste("AllFreq ~ ", paste(comp_columns, collapse = " + "), "+ Condition(Latitude + Longitude)")
