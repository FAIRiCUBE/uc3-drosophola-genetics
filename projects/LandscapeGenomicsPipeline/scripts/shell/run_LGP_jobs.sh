...
echo "READING GENOMIC REGIONS"
#genomic_regions=($(gunzip -c /media/inter/ssteindl/FC/usecaserepo/uc3-drosophola-genetics/projects/LandscapeGenomicsPipeline/data/PoolSeq2023.vcf.gz | awk '!/^#/ {print $1}' | uniq))
###needs very long, maybe improve this!!
genomic_regions=($(gunzip -c $VCFFILE | awk '/^[^#]/ {print $1}' | sort -u))
#genomic_regions=($(gunzip -c "$input" | awk '/^[^#]/ {print $1}' | sort -u))
cd ..
