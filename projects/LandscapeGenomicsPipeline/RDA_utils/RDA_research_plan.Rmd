...
WP_path <-"/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/WormPickerOOP/use_case/wormpicked_data/rasdaman_data_test.csv"
WP_data <- read.csv(WP_path, header=T)
WP_Null_path <- "/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/WormPickerOOP/data/NullValues_05_12_24.csv"
...
RasdaERA5 <- colnames(Env85)[51:57]
Env85 <-  Env85[, !colnames(Env85) %in% RasdaERA5]
colnames(Env85) <- gsub("\\.y","",colnames(Env85))
###work with this
...
Date_num <- as.numeric(as.Date(EnvCore$date, format = "%Y-%m-%d"))
...
nc <- sapply(names(EnvCore_imputed), rename, patterns = patterns, replacements = replacements)
colnames(EnvCore_imputed) <- nc
exclude <- c("SMcF", "SMcSens", "SMpF", "ERA5stype")
DATA <- EnvCore_imputed[, ! colnames(EnvCore_imputed) %in% exclude]
...
ROWS <- WC[rownames(WC) %in% rownames(DATA),]
...
write.csv(DATA, file="/media/inter/ssteindl/FC/usecaserepo/SYNC0524/uc3-drosophola-genetics/projects/ClimateData/Env.csv")
